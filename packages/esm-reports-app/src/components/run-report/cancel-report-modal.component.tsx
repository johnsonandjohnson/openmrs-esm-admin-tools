...
const CancelReportModal: React.FC<CancelReportModalProps> = ({
  closeModal,
  reportRequestUuid,
  modalType,
}) => {
  ...
  const getModalTitleByType = (modalType) => {
    if (modalType === "delete") {
      ...
      return t("deleteSchedule", "Delete Schedule");
    }
  };
  ...
  const getSuccessToastMessageByType = (modalType) => {
    if (modalType === "delete") {
      ...
      return t(
        "reportScheduleDeletedSuccessfully",
  ...
  const getFailedToastMessageByType = (modalType) => {
    if (modalType === "delete") {
      ...
      return t(
        "reportScheduleDeletingErrorMsg",
        "Error during report schedule deleting"
      );
    }
  };
    ...
    <div>
      ...
      <ModalFooter>
        ...
        <Button kind="danger" onClick={handleCancel} disabled={isCanceling}>
          {isCanceling ? (
            <InlineLoading
              description={getLoadingMessageByType(modalType) + "..."}
            />
          ) : (
            <span>{t("yes", "Yes")}</span>
          )}
